...
PRD Annotator 2.5.0 只用于原型 HTML 标注。新建和编辑标注使用标题、说明、类型、PRD 内容、备注五个字段；历史字段和未知扩展字段在同步过程中继续保留。页面 PRD、页面字段规范、页面接口文档和本页补充资料按逻辑页面隔离，全局文档与待关联候选通过关联文档入口查看。浏览器展示层不负责写项目文件，也不启动 Python、Node、扩展、云端或本地保存服务。浏览器把未同步标注保存在 `localStorage`；具备项目写权限的 AI Agent 负责把标注、路由注册和文档资产持久化。标注同步本身不授权修改页面 PRD、总 PRD、字段规范或接口文档。
