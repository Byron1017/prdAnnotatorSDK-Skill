...
PRD Annotator 2.5.1 只用于原型 HTML 标注。新建和编辑标注使用标题、说明、类型、PRD 内容、备注五个字段；历史字段和未知扩展字段在同步过程中继续保留。页面 PRD、页面字段规范、页面接口文档和本页补充资料按逻辑页面隔离，全局文档与待关联候选通过关联文档入口查看。浏览器展示层不负责写项目文件，也不启动 Python、Node、扩展、云端或本地保存服务。浏览器把未同步标注保存在 `localStorage`；具备项目写权限的 AI Agent 负责把标注、路由注册和文档资产持久化。标注同步本身不授权修改页面 PRD、总 PRD、字段规范或接口文档。
...
2. `页面 PRD`：当前页 PRD 正文；顶部二级切换可立即打开 `本页补充资料 <数量>`，无需先滚动完长 PRD。
3. `页面字段规范`：只显示明确关联当前逻辑页的字段规范。
4. `页面接口文档`：只显示明确关联当前逻辑页的接口文档。
5. `关联文档`：最后一个入口页，固定提供 `总需求文档`、`总 PRD 文档`、`总字段规范`、`总接口文档` 四张可点击卡片。

每张全局卡片的详情严格分为 `全局文档` 与 `待关联候选`，并提供 `返回文档入口`。页面文档不能进入全局入口；待关联候选不能冒充全局文档。SDK 不合并、不替用户选择文档。
...
安装 SDK、创建标注、同步标注、刷新路由或刷新 View 都不构成文档写入或重新归属授权。只有用户明确提出创建或更新页面 PRD、总 PRD、页面/总字段规范、页面/总接口文档或其他关联文档时，AI Agent 才能写源文档。

授权后的流程：

1. 读取当前逻辑页 JSON 和 Manifest 中全部文档资产。
2. 查找同类文档的目录、命名、格式、章节、表格和术语。
3. 分别确认类型与范围：明确的当前页/指定页请求关联该逻辑页；明确的项目级请求为全局；范围不清时保留为待关联候选。目录接近、文件名相似、当前打开页面都不是归属证据。
4. 使用用户指定目标；只有一个同类型、同范围的明确模板时沿用；存在多个候选或页面/全局都可能时先列出并询问。
5. 只写用户要求的文档，保留其他候选和人工映射。
6. 页面级影响只更新页面 PRD；公共规则、跨页面流程或总体范围同时更新已经明确的总 PRD。总 PRD 不明确时询问。
7. 刷新 Manifest 和 View，执行 `check-project.mjs`，报告修改文件、范围、页面 ID、Drawer 位置与内容摘要。

文档生成后，页面 PRD、页面字段规范和页面接口文档分别进入对应页面 Tab；全局文档进入对应总文档卡片；未明确归属的资产只进入 `待关联候选`。用户仍可以查看全部候选并自行决定后续合并或取舍。
